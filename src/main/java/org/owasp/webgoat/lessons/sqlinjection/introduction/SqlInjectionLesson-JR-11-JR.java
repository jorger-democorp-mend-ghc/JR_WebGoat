...
public class SqlInjectionLesson9 extends AssignmentEndpoint {
  ...
  @ResponseBody
  public AttackResult handleSearch(@RequestParam String name, @RequestParam String auth_tan) {
    return unsafeEmployeeLookup(name, auth_tan);
  }

  protected AttackResult unsafeEmployeeLookup(String name, String auth_tan) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      try {
        ...
        recordAccessLog(connection, query);
        ResultSet results = statement.executeQuery(query);

        if (results.getStatement() != null) {
          if (results.first()) {
            output.append(formatResultsAsTable(results));
            results.last();
            ...
            // no results
            return failed(this).feedback("sql-injection.9.no.results").build();
          }
        } else {
  ...
  public static String formatResultsAsTable(ResultSet results) throws SQLException {
  ...
  public static void recordAccessLog(Connection connection, String action) {
