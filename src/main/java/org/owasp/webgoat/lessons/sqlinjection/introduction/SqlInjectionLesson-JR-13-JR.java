...
public class SqlInjectionLesson10 extends AssignmentEndpoint {
  ...
  @ResponseBody
  public AttackResult processLoginRequest(@RequestParam String name, @RequestParam String auth_tan) {
    return vulnerableUserQuery(name, auth_tan);
  }

  protected AttackResult vulnerableUserQuery(String name, String auth_tan) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      try {
        ...
        writeAuditLog(connection, query);
        ResultSet results = statement.executeQuery(query);

        if (results.getStatement() != null) {
          if (results.first()) {
            output.append(buildResultTable(results));
            results.last();
            ...
            if (results.getRow() > 1) {
              ...
              // only one record
              return failed(this).feedback("sql-injection.10.one").output(output.toString()).build();
            }
  ...
  public static String buildResultTable(ResultSet results) throws SQLException {
  ...
  public static void writeAuditLog(Connection connection, String action) {
